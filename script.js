...
// ✅ Drag-and-Drop Logic for Timeline
    ...
    document.getElementById("proceed-after-timeline")?.addEventListener("click", function () {
        showPage("final-section");
    });
  // ✅ Handle Page Transition to Challenge 4 Intro
document.getElementById("start-challenge-4")?.addEventListener("click", function () {
    showPage("challenge-4-intro");
});

// ✅ Handle Page Transition to Challenge 4 Map Challenge
document.getElementById("start-challenge-4-map")?.addEventListener("click", function () {
    showPage("challenge-4-map"); // This will be the next part where students interact with the map
});
  document.getElementById("submit-map-answers").addEventListener("click", function () {
    let correctAnswers = {
        question1: "break through Allied lines and capture Paris",
        question2: "Battle of Amiens",
        question3: "The Hindenburg Line"
    };
    ...
    for (let key in correctAnswers) {
        if (userAnswers[key] !== correctAnswers[key]) {
            allCorrect = false;
            feedbackText = "❌ Incorrect answers! Try again.";
        }
    }

    if (allCorrect) {
        feedbackText = "✅ Correct! You've successfully analyzed the map.";
        document.getElementById("proceed-to-challenge-5").classList.remove("hidden");
    }

    let feedbackElement = document.getElementById("map-feedback");
    feedbackElement.innerText = feedbackText;
    feedbackElement.classList.remove("hidden");
});

// Proceed to Challenge 5
document.getElementById("proceed-to-challenge-5").addEventListener("click", function () {
